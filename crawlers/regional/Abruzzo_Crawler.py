...
class AbruzzoCrawler(BaseCrawler):
    """
    Crawler for Regione Abruzzo grants.
    """
    
    def __init__(self, max_pages: int = 10, delay: float = 1.0):
        """
        Initialize the Abruzzo crawler.
        ...
        self.logger = logging.getLogger("AbruzzoCrawler")
    
    def get_grant_listing_urls(self) -> List[str]:
        """
        Get URLs for individual grant listings from Regione Abruzzo.
        
        Returns:
            List[str]: List of URLs to individual grant pages
        """
        # Abruzzo likely has a bandi page
        grants_url = "/bandi-e-contributi"  # Adjust based on actual site structure
    ...
    def parse_grant_details(self, url: str) -> Dict[str, Any]:
        """
        Parse the details of a grant from Regione Abruzzo.
        ...
        procedure_element = soup.select_one(".procedura-presentazione")  # Adjust selector
    ...
    def _get_full_grant_text(self, soup: BeautifulSoup) -> str:
        ...
        procedure = soup.select_one(".procedura-presentazione")  # Adjust selector
        if procedure:
