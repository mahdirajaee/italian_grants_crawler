...
class BasilicataCrawler(BaseCrawler):
    """
    Crawler for Regione Basilicata grants.
    """
    
    def __init__(self, max_pages: int = 10, delay: float = 1.0):
        """
        Initialize the Basilicata crawler.
        ...
        """
        super().__init__(
            base_url="https://portalebandi.regione.basilicata.it",
            max_pages=max_pages,
            delay=delay
        )
        self.logger = logging.getLogger("BasilicataCrawler")
    
    def get_grant_listing_urls(self) -> List[str]:
        """
        Get URLs for individual grant listings from Regione Basilicata.
        
        Returns:
            List[str]: List of URLs to individual grant pages
        """
        # Basilicata has a dedicated portal for bandi
        grants_url = "/PortaleBandi/home.jsp"  # Adjust based on actual site structure
        ...
        for item in grant_items:
            link = item.select_one("a.dettaglio-link")  # Adjust selector
    ...
    def parse_grant_details(self, url: str) -> Dict[str, Any]:
        """
        Parse the details of a grant from Regione Basilicata.
