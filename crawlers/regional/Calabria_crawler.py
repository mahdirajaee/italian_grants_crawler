...
class CalabriaCrawler(BaseCrawler):
    """
    Crawler for Regione Calabria grants (via Calabria Europa portal).
    """
    
    def __init__(self, max_pages: int = 10, delay: float = 1.0):
        """
        Initialize the Calabria crawler.
        ...
        """
        super().__init__(
            base_url="https://calabriaeuropa.regione.calabria.it",
            max_pages=max_pages,
            delay=delay
        )
        self.logger = logging.getLogger("CalabriaCrawler")
    
    def get_grant_listing_urls(self) -> List[str]:
        """
        Get URLs for individual grant listings from Regione Calabria.
        
        Returns:
            List[str]: List of URLs to individual grant pages
        """
        # Calabria uses a dedicated portal for EU funds
    ...
    def parse_grant_details(self, url: str) -> Dict[str, Any]:
        """
        Parse the details of a grant from Regione Calabria.
