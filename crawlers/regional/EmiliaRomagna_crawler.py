...
class EmiliaRomagnaCrawler(BaseCrawler):
    """
    Crawler for Regione Emilia-Romagna grants.
    """
    
    def __init__(self, max_pages: int = 10, delay: float = 1.0):
        """
        Initialize the Emilia-Romagna crawler.
        ...
        """
        super().__init__(
            base_url="https://imprese.regione.emilia-romagna.it",
            max_pages=max_pages,
            delay=delay
        )
        self.logger = logging.getLogger("EmiliaRomagnaCrawler")
    
    def get_grant_listing_urls(self) -> List[str]:
        """
        Get URLs for individual grant listings from Regione Emilia-Romagna.
        
        Returns:
            List[str]: List of URLs to individual grant pages
        """
        # Emilia-Romagna uses a dedicated site for business grants
        ...
        grant_items = soup.select(".item-bando")  # Adjust based on actual site structure
    ...
    def _extract_grant_urls_from_page(self, soup: BeautifulSoup) -> List[str]:
        ...
        # Find grant links - adjust selector based on site structure
        grant_items = soup.select(".item-bando")  # Adjust selector
    ...
    def parse_grant_details(self, url: str) -> Dict[str, Any]:
        """
        Parse the details of a grant from Regione Emilia-Romagna.
        ...
        desc_element = soup.select_one(".abstract")  # Adjust selector
        if desc_element:
            grant_data["Descrizione breve (Plain text)"] = desc_element.text.strip()
        
        # Extract full description
        full_desc_element = soup.select_one(".testo-bando")  # Adjust selector
        ...
        # Extract deadlines
        deadline_element = soup.select_one(".scadenza")  # Adjust selector
        if deadline_element:
        ...
        procedure_element = soup.select_one(".come-partecipare")  # Adjust selector
        ...
        funding_element = soup.select_one(".risorse-disponibili")  # Adjust selector
        if funding_element:
    ...
    def _get_full_grant_text(self, soup: BeautifulSoup) -> str:
        ...
        desc = soup.select_one(".abstract")  # Adjust selector
        if desc:
            text_parts.append(desc.text.strip())
        
        # Get full description
        full_desc = soup.select_one(".testo-bando")  # Adjust selector
        if full_desc:
        ...
        procedure = soup.select_one(".come-partecipare")  # Adjust selector
        if procedure:
