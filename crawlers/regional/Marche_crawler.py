...
class MarcheCrawler(BaseCrawler):
    """
    Crawler for Regione Marche grants.
    """
    
    def __init__(self, max_pages: int = 10, delay: float = 1.0):
        """
        Initialize the Marche crawler.
        ...
        self.logger = logging.getLogger("MarcheCrawler")
    
    def get_grant_listing_urls(self) -> List[str]:
        """
        Get URLs for individual grant listings from Regione Marche.
        
        Returns:
            List[str]: List of URLs to individual grant pages
        """
        # Marche likely has a bandi page
        ...
        grant_items = soup.select(".box-bando")  # Adjust based on actual site structure
    ...
    def _extract_grant_urls_from_page(self, soup: BeautifulSoup) -> List[str]:
        ...
        # Find grant links - adjust selector based on site structure
        grant_items = soup.select(".box-bando")  # Adjust selector
    ...
    def parse_grant_details(self, url: str) -> Dict[str, Any]:
        """
        Parse the details of a grant from Regione Marche.
        ...
        full_desc_element = soup.select_one(".dettaglio-bando")  # Adjust selector
    ...
    def _get_full_grant_text(self, soup: BeautifulSoup) -> str:
        ...
        full_desc = soup.select_one(".dettaglio-bando")  # Adjust selector
        if full_desc:
