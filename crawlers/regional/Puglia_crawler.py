...
class PugliaCrawler(BaseCrawler):
    """
    Crawler for Regione Puglia grants.
    """
    
    def __init__(self, max_pages: int = 10, delay: float = 1.0):
        """
        Initialize the Puglia crawler.
        ...
        self.logger = logging.getLogger("PugliaCrawler")
    
    def get_grant_listing_urls(self) -> List[str]:
        """
        Get URLs for individual grant listings from Regione Puglia.
        
        Returns:
            List[str]: List of URLs to individual grant pages
        """
        # Puglia likely has a bandi page
        grants_url = "/web/bandi-e-avvisi"  # Adjust based on actual site structure
    ...
    def parse_grant_details(self, url: str) -> Dict[str, Any]:
        """
        Parse the details of a grant from Regione Puglia.
    ...
    def _get_full_grant_text(self, soup: BeautifulSoup) -> str:
        ...
        procedure = soup.select_one(".procedura-domanda")  # Adjust selector
        if procedure:
