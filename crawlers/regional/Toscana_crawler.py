...
class ToscanaCrawler(BaseCrawler):
    """
    Crawler for Regione Toscana grants.
    """
    
    def __init__(self, max_pages: int = 10, delay: float = 1.0):
        """
        Initialize the Toscana crawler.
        ...
        self.logger = logging.getLogger("ToscanaCrawler")
    
    def get_grant_listing_urls(self) -> List[str]:
        """
        Get URLs for individual grant listings from Regione Toscana.
        
        Returns:
            List[str]: List of URLs to individual grant pages
        """
        # Toscana likely has a bandi page
        grants_url = "/it/bandi-aperti"  # Adjust based on actual site structure
    ...
    def parse_grant_details(self, url: str) -> Dict[str, Any]:
        """
        Parse the details of a grant from Regione Toscana.
        ...
        desc_element = soup.select_one(".sommario")  # Adjust selector
        if desc_element:
    ...
    def _get_full_grant_text(self, soup: BeautifulSoup) -> str:
        ...
        desc = soup.select_one(".sommario")  # Adjust selector
        if desc:
