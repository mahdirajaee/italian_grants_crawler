...
class TrentinoCrawler(BaseCrawler):
    """
    Crawler for Provincia Autonoma di Trento grants.
    """
    
    def __init__(self, max_pages: int = 10, delay: float = 1.0):
        """
        Initialize the Trentino crawler.
        ...
        """
        super().__init__(
            base_url="https://www.provincia.tn.it",
            max_pages=max_pages,
            delay=delay
        )
        self.logger = logging.getLogger("TrentinoCrawler")
    
    def get_grant_listing_urls(self) -> List[str]:
        """
        Get URLs for individual grant listings from Provincia Autonoma di Trento.
        
        Returns:
            List[str]: List of URLs to individual grant pages
        """
        # Trentino likely has a page for grants/bandi
        grants_url = "/contributi-finanziamenti"  # Adjust based on actual site structure
        ...
        grant_items = soup.select(".bando-item")  # Adjust based on site structure
    ...
    def parse_grant_details(self, url: str) -> Dict[str, Any]:
        """
        Parse the details of a grant from Provincia Autonoma di Trento.
        ...
        grant_data = {
            ...
            "Emanazione": "Regionale",  # For Trentino, which is an autonomous province
            "Località_MR": "Trentino Alto Adige",
        ...
        funding_element = soup.select_one(".risorse-finanziarie")  # Adjust selector
        if funding_element:
    ...
    def _get_full_grant_text(self, soup: BeautifulSoup) -> str:
        ...
        full_desc = soup.select_one(".descrizione-completa")  # Adjust selector
        if full_desc:
