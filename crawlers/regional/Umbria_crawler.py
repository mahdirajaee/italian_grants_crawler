...
class UmbriaCrawler(BaseCrawler):
    """
    Crawler for Regione Umbria grants.
    """
    
    def __init__(self, max_pages: int = 10, delay: float = 1.0):
        """
        Initialize the Umbria crawler.
        ...
        self.logger = logging.getLogger("UmbriaCrawler")
    
    def get_grant_listing_urls(self) -> List[str]:
        """
        Get URLs for individual grant listings from Regione Umbria.
        
        Returns:
            List[str]: List of URLs to individual grant pages
        """
        # Umbria likely has a bandi page
        ...
        grant_items = soup.select(".bando-listing-item")  # Adjust based on actual site structure
        
        for item in grant_items:
            link = item.select_one("a.link-bando")  # Adjust selector
    ...
    def _extract_grant_urls_from_page(self, soup: BeautifulSoup) -> List[str]:
        ...
        # Find grant links - adjust selector based on site structure
        grant_items = soup.select(".bando-listing-item")  # Adjust selector
    ...
    def parse_grant_details(self, url: str) -> Dict[str, Any]:
        """
        Parse the details of a grant from Regione Umbria.
        ...
        desc_element = soup.select_one(".sommario-bando")  # Adjust selector
        if desc_element:
        ...
        opening_date_element = soup.select_one(".apertura-bando")  # Adjust selector
        ...
        eligible_element = soup.select_one(".destinatari-bando")  # Adjust selector
        if eligible_element:
            grant_data["A chi si rivolge"] = eligible_element.text.strip()
        
        # Extract application procedure
        procedure_element = soup.select_one(".modalita-partecipazione")  # Adjust selector
    ...
    def _get_full_grant_text(self, soup: BeautifulSoup) -> str:
        ...
        desc = soup.select_one(".sommario-bando")  # Adjust selector
        if desc:
            text_parts.append(desc.text.strip())
        
        # Get full description
        full_desc = soup.select_one(".contenuto-bando")  # Adjust selector
        if full_desc:
            text_parts.append(full_desc.text.strip())
        
        # Get eligible applicants
        eligible = soup.select_one(".destinatari-bando")  # Adjust selector
        if eligible:
            text_parts.append(eligible.text.strip())
        
        # Get application procedure
        procedure = soup.select_one(".modalita-partecipazione")  # Adjust selector
        if procedure:
