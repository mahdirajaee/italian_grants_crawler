...
class VenetoCrawler(BaseCrawler):
    """
    Crawler for Regione Veneto grants.
    """
    
    def __init__(self, max_pages: int = 10, delay: float = 1.0):
        """
        Initialize the Veneto crawler.
        ...
        self.logger = logging.getLogger("VenetoCrawler")
    
    def get_grant_listing_urls(self) -> List[str]:
        """
        Get URLs for individual grant listings from Regione Veneto.
        
        Returns:
            List[str]: List of URLs to individual grant pages
        """
        # Veneto likely has a main bandi page
        ...
        # Find grant links - selector will need adjustment
        grant_elements = soup.select(".bando-item")  # Adjust based on actual site structure
        
        for element in grant_elements:
            link = element.select_one("a.bando-link")  # Adjust based on site structure
    ...
    def _extract_grant_urls_from_page(self, soup: BeautifulSoup) -> List[str]:
        ...
        # Find grant links - selector needs adjustment
        grant_elements = soup.select(".bando-item")  # Adjust based on site structure
    ...
    def parse_grant_details(self, url: str) -> Dict[str, Any]:
        """
        Parse the details of a grant from Regione Veneto.
        ...
        procedure_element = soup.select_one(".iter-domanda")  # Adjust selector
    ...
    def _map_grant_type(self, type_text: str) -> str:
        ...
        if any(keyword in type_text for keyword in ["sportello", "a sportello", "procedura sportello"]):
    ...
    def _get_full_grant_text(self, soup: BeautifulSoup) -> str:
        ...
        procedure = soup.select_one(".iter-domanda")  # Adjust selector
        if procedure:
