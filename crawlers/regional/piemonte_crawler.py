...
class PiemonteCrawler(BaseCrawler):
    """
    Crawler for Regione Piemonte grants.
    """
    
    def __init__(self, max_pages: int = 10, delay: float = 1.0):
        """
        Initialize the Piemonte crawler.
        ...
        self.logger = logging.getLogger("PiemonteCrawler")
    
    def get_grant_listing_urls(self) -> List[str]:
        """
        Get URLs for individual grant listings from Regione Piemonte.
        
        Returns:
            List[str]: List of URLs to individual grant pages
        """
        # The Piemonte site likely has a main bandi page
        grants_url = "/finanziamenti"  # Adjust based on actual site structure
        ...
        # Find grant links - selector will need to be adjusted based on the HTML structure
        grant_cards = soup.select(".card-bando")  # Adjust based on actual site structure
        
        for card in grant_cards:
            # Look for detail links
            link = card.select_one("a.dettaglio-bando")  # Adjust selector
    ...
    def _extract_grant_urls_from_page(self, soup: BeautifulSoup) -> List[str]:
        ...
        # Find grant links - selector needs adjustment based on the HTML structure
        grant_cards = soup.select(".card-bando")  # Adjust selector
    ...
    def parse_grant_details(self, url: str) -> Dict[str, Any]:
        """
        Parse the details of a grant from Regione Piemonte.
        ...
        target_element = soup.select_one(".destinatari-bando")  # Adjust selector
        if target_element:
            grant_data["A chi si rivolge"] = target_element.text.strip()
    ...
    def _map_grant_type(self, type_text: str) -> str:
        ...
        if any(keyword in type_text for keyword in ["sportello", "chiusura sportello"]):
    ...
    def _get_full_grant_text(self, soup: BeautifulSoup) -> str:
        ...
        # Get all content from content section
